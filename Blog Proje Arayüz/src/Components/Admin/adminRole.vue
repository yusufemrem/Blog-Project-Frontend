      ...
      <body>
        ...
        <div class="container-fluid">
          <main class="tm-main">
            ...
            <hr />
            <div>
              <form class="" v-if="showAddRoleForm">
...
export default {
  ...
  methods: {
    ...
    saverole() {
      this.$store.dispatch("saverole", this.role);
      this.showAddRoleForm = true;
    },
    fetchAllRole() {
      this.$http
        .get(`https://localhost:44392/api/Role/Index`)
        .then((response) => {
    ...
    deleteRole(id) {
      ...
      this.$http
        .delete("https://localhost:44392/api/Role/DeleteRole/" + id)
        .then(() => {
          // Remove the deleted staff member from the staffList array
          this.allRole = this.allRole.filter((role) => role.id !== id);
        })
        .catch((error) => {
